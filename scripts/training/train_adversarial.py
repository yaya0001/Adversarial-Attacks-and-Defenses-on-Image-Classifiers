...
=============================
Trains a SimpleCNN on MNIST using adversarial training (AT).
...
In both cases, every mini-batch is replaced by its adversarial counterpart
before computing the loss and updating parameters.  This implements the
min-max formulation of Madry et al. (2018):

    min_θ  E_{(x,y)~D} [ max_{||δ||∞ ≤ ε}  L(θ, x+δ, y) ]

The inner maximization is approximated by FGSM or PGD.

Usage:
    python train_adversarial.py --mode fgsm-at
    python train_adversarial.py --mode pgd-at
...
PGD_STEPS = 7              # Number of PGD inner iterations (standard choice)
...
def train_adversarial(mode: str) -> None:
    """
    Full adversarial training pipeline.
    ...
    metrics_name = f"training_metrics_{mode.replace('-', '_')}.json"

    print(f"\n{'='*60}")
    print(f"  Adversarial Training — {mode_label}")
    print(f"  ε = {AT_EPSILON}, Epochs = {EPOCHS}")
    if mode == "pgd-at":
        print(f"  PGD: α = {PGD_ALPHA}, steps = {PGD_STEPS}")
    print(f"{'='*60}\n")
    ...
    for epoch in range(EPOCHS):
        ...
        for images, labels in tqdm(train_loader,
            ...
            # model must be in eval mode during attack generation
            # to avoid interference from dropout/BN (our model has none,
            # but this is correct practice)
            model.eval()
            if mode == "fgsm-at":
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Adversarial Training for SimpleCNN on MNIST"
    )
    parser.add_argument(
        "--mode",
        type=str,
    ...
    print("\n✓ All adversarial training complete.")
